...
{
    public AudioSpectrum audioAnalyzer;
        ...
        if (offsetTimer >= analysisOffset)
        {
            for (int i = 0; i < numberOfCubes; i++)
            {
                if (cubes[i] != null) // Check if cube is null before updating
                {
                    ...
                    Vector3 newScale = new Vector3(Mathf.Min(averageAmplitude * cubeHeightMultiplier * Mathf.Max((i + 1), 100), maxHeight), cubeSpacing, cubeSpacing);
